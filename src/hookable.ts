...
> {
  ...
  callHookWith<NameT extends HookNameT> (callFn: (hooks: HookCallback[], ...args: Parameters<InferCallback<HooksT, NameT>>) => any, name: NameT, ...args: Parameters<InferCallback<HooksT, NameT>>) {
    if (!this._hooks[name]) {
      return
    }
